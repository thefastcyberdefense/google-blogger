...
function installSvg(output:HTMLElement,markup:string){
 ...
 // Defense-in-depth on the exact library output; no author HTML is inserted directly.
 if(doc.querySelector('script,foreignObject,iframe,object,embed,image'))throw new Error('Unsupported active diagram content');
 for(const el of Array.from(svg.querySelectorAll('*')).concat([svg])){
 ...
 const imported=document.importNode(svg,true);output.replaceChildren(imported);
}
function schedule(state:State,retry=false){
 state.revision++;if(state.queued)return;
 state.queued=true;state.status.textContent='Rendering diagram…';state.retry.disabled=true;
 serial=serial.then(async()=>{
  let seen=-1;
  try{
   const source=prepareDiagram(state.source,state.legacy);const api=await loadMermaid(retry);
   do{
    seen=state.revision;api.initialize(diagramConfig(effectiveTheme()));
    await api.parse(source);const {svg}=await api.render(`fcd-diagram-${++counter}`,source);
    if(seen!==state.revision)continue;
    installSvg(state.output,svg);state.output.hidden=false;state.figure.dataset.state='rendered';state.scale=1;state.output.style.width='100%';
  ...
  finally{state.queued=false;state.retry.disabled=false;state.retry.textContent='Render again';}
 }).catch(()=>{state.queued=false;state.retry.disabled=false;state.status.textContent='Diagram unavailable; use its source.';});
}
export function initDiagrams(root:ParentNode=document){
 ...
 if(blocks.size&&!listening){listening=true;document.addEventListener('fcd:theme-change',()=>{for(const state of active)if(state.figure.isConnected&&state.figure.dataset.state==='rendered'||state.queued)schedule(state);});}
}
