...
// A UI timeout does not abort import(). Keep that underlying request single-flight.
let imported:Promise<{default:MermaidAPI}>|undefined;
export const loadMermaid=createSingleFlight(async()=>{
 if(!imported){imported=(import(/* @vite-ignore */ MERMAID_URL) as Promise<{default:MermaidAPI}>).catch(error=>{imported=undefined;throw error;});}
