import {parseDiscovery,type DiscoveryPost} from './feed.ts';
export function createFeedClient(base:string,limit:8|50,fetcher:typeof fetch=fetch){
 ...
 async function request(){
  ...
  const work=async()=>{
   const response=await fetcher(endpoint.href,{credentials:'omit',redirect:'error',signal:controller.signal});
   ...
   if(Number(response.headers.get('content-length'))>500000)throw new Error('Feed too large');
   reader=response.body?.getReader();if(!reader)throw new Error('Missing feed body');
   const chunks:Uint8Array[]=[];let bytes=0;
   while(true){const {value,done}=await reader.read();if(done)break;bytes+=value.byteLength;if(bytes>500000)throw new Error('Feed too large');chunks.push(value);}
 ...
 return {
  canRetry:()=>failed&&!busy&&attempts===1,
  load(retry=false):Promise<DiscoveryPost[]>{
   if(!pending||retry&&failed&&!busy&&attempts===1){attempts++;busy=true;failed=false;pending=request().catch(error=>{failed=true;throw error;}).finally(()=>{busy=false;});}
   return pending;
  }
 };
}
