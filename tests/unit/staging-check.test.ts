...
function markup(type:string){
 ...
 return `<html><head><meta name="theme-build" content="${build}"><link rel="canonical" href="${origin}/${type}"></head><body><main id="content">${content}</main></body></html>`;
}
...
async function checkOne(type:string,html:string){
 const manifest=validateManifest(raw());manifest.views=manifest.views.filter(v=>v.type===type);
 const fetch=vi.spyOn(globalThis,'fetch').mockResolvedValue(new Response(html,{status:type==='error'?404:200}));
 try{await runStaging(manifest);}finally{fetch.mockRestore();}
}
it('rejects a populated catalog masquerading as the article view',async()=>{
 await expect(checkOne('article',markup('home'))).rejects.toThrow(/article/i);
},30000);
it('rejects a hidden article body even when a visible catalog sibling exists',async()=>{
 const html=markup('article').replace('id="article-body"','id="article-body" hidden').replace('</main>',card+'</main>');
 await expect(checkOne('article',html)).rejects.toThrow(/article/i);
},30000);
...
it.each(hiddenCases)('rejects $name in $type views',async({type,name})=>{
 await expect(checkOne(type,mutations[name](markup(type)))).rejects.toThrow();
},30000);
